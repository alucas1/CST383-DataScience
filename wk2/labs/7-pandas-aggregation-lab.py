...
#   Hard way: Get columns with numbers and use agg 
num_cols = df.select_dtypes(include=[np.number]).columns 
df[num_cols].agg(['min', 'max', 'mean'])
#   Or the alternative easy way with extra data
df.describe()
...
#   harder way: group on education values and display education column only, then count and sort
df.groupby(['education'])['education'].count().sort_values()  
...
age_groups = ['>40' if (x > 40) else '<=40' for x in df.age]
...
#   Or using lambda instead
df_high_ed_occupations = df.groupby('occupation').filter(lambda x: x.education_num.mean() > 10)
...
#   alternate solution: count num of unique values, and only get the index(unique countries)
...
# a name.
#   Checks if the size of country_names is the same as the unique 2-letter country_names
#   if true, then all countries have a unique 2-letter identification. else false
